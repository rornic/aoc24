...
fn parse_input(input: &str) -> Vec<Vec<u32>> {
    ...
    for line in input.lines() {
        let levels = line.split(" ").map(|level| level.parse::<u32>().unwrap()).collect();
        reports.push(levels);
    }
    reports
...
impl Problem for Day2 {
    fn part1(&self, input: &str) -> String {
        ...
        let safe = reports.into_iter().filter(|report| is_report_safe(report)).count();
        format!("{}", safe)
    }

    fn part2(&self, input: &str) -> String {
        let reports = parse_input(input);
        let safe = reports.into_iter().map(report_permutations).filter(|reports| reports.iter().any(|report| is_report_safe(report))).count();
        format!("{}", safe)
    }
}
